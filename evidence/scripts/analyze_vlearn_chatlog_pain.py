...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default=DEFAULT_CSV,
        help=(
            "Path to the original VLearn chatlog CSV. The final submission repo "
            "does not include the data pack, so pass this explicitly when auditing."
        ),
    )
    ...
    if not args.csv.exists():
        raise SystemExit(
            f"CSV not found: {args.csv}. The final submission intentionally does "
            "not include the original data pack; rerun with --csv pointing to an "
