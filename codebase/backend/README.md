...
`POST /api/v2/chat` là endpoint non-streaming dùng cùng orchestration với stream. Active flow có năm mode nội bộ:
...
Các API upload/list/file/delete/status document và delete conversation được giữ vì frontend dùng trực tiếp.
...
Chưa có OCR riêng cho scanned PDF, authentication hoặc production deployment. Summary, quiz, flashcard, chat v1 và AI hiểu nét vẽ không thuộc active CP4/CP5 scope.
