...
- Một case fail của nhóm: Streaming có thể tạo trải nghiệm khó hiểu nếu lỗi xảy ra sau khi đã gửi delta đầu hoặc khi đổi tài liệu giữa chừng.
- Cách nhóm xử lý: Frontend abort stream khi reset/đổi document, backend chỉ fallback provider trước delta đầu tiên, còn lỗi sau phản hồi đang dở thì báo retry thay vì ghép hai provider.
